...
import { Search as SearchIcon, SlidersHorizontal, Clock, TrendingUp, MapPin, Phone, Star, Tag, Map, List } from 'lucide-react';
...
const Search = () => {
  ...
  const communityTagOptions = ['Women-Owned', 'Family-Owned', 'Vegan-Friendly', 'Kid-Friendly', 'LGBTQ-Owned', 'New Business'];
  const serviceOptions = ['Delivery', 'Online Booking', 'Braiding', 'Takeout', 'Walk-ins', 'Appointments', 'Catering'];
  const trendingSearches = ['Soul Food', 'Hair Services', 'Tech Startups', 'Fashion Boutiques', 'Wellness Centers'];

  useEffect(() => { loadBusinesses(); }, [searchParams]);
  useEffect(() => { applyFiltersAndSort(); }, [businesses, filters, sortBy]);
            ...
            <div className="suggestions-panel">
              {recentSearches.length > 0 && (
                <div className="suggestions-section">
                  ...
                  {recentSearches.map((search, idx) => (
                    <button key={idx} className="suggestion-item" onClick={() => performSearch(search)}>
              ...
              <div className="suggestions-section">
                ...
                {trendingSearches.map((trend, idx) => (
                  <button key={idx} className="suggestion-item" onClick={() => performSearch(trend)}>
            ...
            <div className="filters-grid">
              ...
              <div className="filter-section">
                ...
                <div className="filter-options">
                  {['all', '1', '5', '10'].map(dist => (
                    <button key={dist} className={`filter-option ${filters.distance === dist ? 'active' : ''}`} onClick={() => handleFilterChange('distance', dist)}>
              ...
              <div className="filter-section">
                ...
                <div className="filter-options">
                  {[0, 3, 4, 5].map(rating => (
                    <button key={rating} className={`filter-option ${filters.minRating === rating ? 'active' : ''}`} onClick={() => handleFilterChange('minRating', rating)}>
                      {rating === 0 ? 'All' : `${rating}★+`}
                    </button>
                  ))}
                </div>
              ...
              <div className="filter-section">
                ...
                <label className="checkbox-label">
                  <input type="checkbox" checked={filters.openNow} onChange={(e) => handleFilterChange('openNow', e.target.checked)} />
                  Open Now
                </label>
              </div>
              <div className="filter-section wide">
                ...
                <div className="tag-options">
                  {communityTagOptions.map(tag => (
                    <button key={tag} className={`tag-option ${filters.communityTags.includes(tag) ? 'active' : ''}`} onClick={() => handleCommunityTagToggle(tag)}>
                      {tag}
                    </button>
                  ))}
                </div>
              ...
              <div className="filter-section">
                <h4>Price Range</h4>
                <select value={filters.priceRange} onChange={(e) => handleFilterChange('priceRange', e.target.value)} className="filter-select">
              ...
              <div className="filter-section">
                <h4>Promotions</h4>
                <label className="checkbox-label">
                  <input type="checkbox" checked={filters.onlyPromotions} onChange={(e) => handleFilterChange('onlyPromotions', e.target.checked)} />
        ...
        <div className="results-header">
          ...
          <div className="results-actions">
            ...
            <button onClick={() => setShowMap(!showMap)} className="toggle-map-btn">
              {showMap ? <><List size={15} /> List</> : <><Map size={15} /> Map</>}
            </button>
          </div>
        </div>
          ...
          <>
            ...
            <div className="results-grid">
              {filteredBusinesses.map((business) => (
                <div key={business.id} className="result-card" onClick={() => navigate(`/business/${business.id}`)}>
                  <div className="card-image-wrapper">
                    ...
                    {business.hasActivePromotions && (
                      <span className="promo-badge"><Tag size={12} /> Promotion</span>
                    )}
                  </div>
                  <div className="result-info">
                    ...
                    {business.rating && (
                      <div className="result-rating"><Star size={13} fill="currentColor" /> {business.rating}</div>
                    )}
                    <p className="result-description">{business.description}</p>
                    <div className="result-details">
                      <p><MapPin size={13} /> {business.address}</p>
                      <p>{business.city}, {business.state} {business.zip_code}</p>
                      {business.phone && <p><Phone size={13} /> {business.phone}</p>}
                    </div>
